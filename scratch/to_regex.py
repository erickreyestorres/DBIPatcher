...
# Специфічні мапінги для виправлення помилок у словнику
SPECIFIC_CORRECTIONS = {
    "EmuMMC включён": "Emummc включён",
    "Форсировано включение USB 3.0": "Форсировано вклчение USB 3.0",
}

def escape_for_regex(text):
    res = re.escape(text)
    res = res.replace(r'\ ', ' ')
    res = re.sub(r' {2,}', r'\\s{2,}', res)
    return res

def transform_to_regex():
    # Тимчасово завантажуємо початкові блоки з тексту користувача (з урахуванням INFO)
    # (Я візьму поточний стан JSON і підкоригую його)
    ...
    for block_id, regex_list in blocks.items():
        # Тут проблема: в blocks.json зараз вже регекси від попереднього проходу.
        # Мені потрібно відтворити список початкових термінів або просто переписати логіку.
        # Спробую використати RAW_BLOCKS з попередніх кроків.
        pass

    # Відтворюю RAW_BLOCKS заново для чистоти
    RAW_BLOCKS_CORRECTED = {
        ...
        "HW_INFO": ["Bluetooth MAC адрес", "Wireless LAN MAC адрес", "Configuration Id1", "Серийный номер", "Код партии батареи", "Экран"],
        "ACTIVITY_INFO": ["RTC запущены (приблизительно)", "Дата первого игрового события", "Общее число игровых сессий", "Число уникальных сыграных игр", "Общее время запущенных игр", "Общее время активной игры"]
    }

    final_regex_blocks = {}
    for b_id, terms in RAW_BLOCKS_CORRECTED.items():
        ...
        for term in terms:
            ...
            if term == "Серийный номер":
                target = next((m for m in matches if "{:s}" in m["suffix"]), None) if b_id == "SD_INFO" else next((m for m in matches if "{:s}" not in m["suffix"]), None)
                ...
                m_list = [m for m in matches if "%" in m["suffix"]] if b_id == "CHARGE_INFO" else [m for m in matches if "%" not in m["suffix"]]
                if m_list: target = sorted(m_list, key=lambda x: len(x["prefix"]), reverse=True)[0]
            ...
            if not target:
                target = next((m for m in matches if "{" in m["suffix"]), matches[0]) if is_info else next((m for m in matches if not m["suffix"]), matches[0])
            
            regex_list.append(escape_for_regex(target["full"]))
        if regex_list:
