...
## [2026-04-27] Автоматизація дистрибуції

### Виконані дії:
1.  **Команда dist**: Реалізовано команду `dist` в `main.py` для автоматичної організації вихідних файлів.
2.  **Структура dist**: Тепер для кожної мови створюється підпапка (наприклад, `dist/ua/`), де лежать перейменовані файли: `DBI.nro` та `translation.bin`.
3.  **Автоматизація**: Команду `dist` додано до повного пайплайну `all`.
4.  **Git**: Папку `dist/` додано в `.gitignore`.

### Комміти:
- `feat: add dist command for automated per-language packaging`
