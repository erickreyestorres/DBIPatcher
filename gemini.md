...
1.  **Команда deploy**: Реалізовано повний цикл деплою:
    ...
    - Завантаження активів (NRO та BIN файли) у реліз.
2.  **Версіонування**: Реалізовано автоматичне ітерування версії (`v0.0.78`) при кожному деплої.
3.  **GitHub CLI**: Інтегровано використання `gh auth token` для автентифікації без ручного введення секретів.

### Комміти:
- `chore: deploy DBI 894 localization (v0.0.78)`
